...
from process_mining.sberpm.metrics._base_metric import BaseMetric
from process_mining.sberpm.metrics._utils import round_decorator
...
class TraceMetric(BaseMetric):
    ...
    def __init__(self, data_holder, time_unit="hour", round_=None):
        ...
        self._traces = DataFrame(
            {self._dh.activity_column: self._grouped_data[self._dh.activity_column].unique()}
        ).set_index(self._dh.activity_column, drop=False)[
            self._dh.activity_column
        ]  # pandas.Series
    ...
    def apply(self):
        ...
        """
        self.metrics = (
            DataFrame(index=self._grouped_data[self._dh.activity_column].unique())
            .join(self.count())
            .join(self.ids())
        ...
        if self._dh.user_column is not None:
            self.metrics = self.metrics.join(self.unique_users()).join(self.unique_users_num())
