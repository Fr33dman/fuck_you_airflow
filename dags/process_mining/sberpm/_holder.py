...
from pandas import (DataFrame, Series, Timedelta, concat, read_csv, read_excel,
                    read_table, to_datetime)
from pandas.api.types import (is_datetime64_any_dtype, is_numeric_dtype,
                              is_string_dtype)

from process_mining.sberpm._utils import generate_data_partitions
...
class DataHolder:
    ...
    keep_weekends: bool, default=True
        Whether to keep or omit in data records on weekends (Sat, Sun).

    skip_holidays_list: tuple[str], default=()
        Collection of manually added holidays; once added, these days will be skipped as weekends.
    ...
    Examples
    --------
    >>> from process_mining.sberpm import DataHolder
    >>> import pandas as pd
    ...
    def __init__(
        ...
        self.skip_holidays_list = self.__parse_provided_holidays_list(skip_holidays_list)
        ...
        if type(data) == str:
            if data.split(".")[-1] == "csv":
                raw_data = read_csv(data, sep=sep, encoding=encoding, nrows=nrows)
            elif data.split(".")[-1] in ["xlsx", "xls"]:
                raw_data = read_excel(data, nrows=nrows)
            elif data.split(".")[-1] == "txt":
                raw_data = read_table(data, sep=sep, encoding=encoding, nrows=nrows)
            else:
                raise ValueError(
        ...
        if duration_column is not None and is_numeric_dtype(data[duration_column]):
            if duration_unit is None:
                raise RuntimeError(
                    'As long as "duration" column is numeric, '
                    '"duration_unit" argument must be set in the constructor of DataHolder.'
                )
            elif duration_unit not in {"second", "minute", "hour", "day"}:
                raise ValueError(
                    f"duration_unit should be one of ['second', 'minute', 'hour', 'day'], "
        ...
        self.data = (
            self._preprocess_data(raw_data, time_format, time_errors, dayfirst, yearfirst)
            if preprocess
            else raw_data
        )
        self.__adjust_calendar_days()
    ...
    def id_column(self):
        if self._id_column is not None:
            ...
            raise LogWithoutIdError("The event log does not have an ID column. This action is impossible.")

    def __parse_provided_holidays_list(self, provided_holidays_list: deque[str]):
        all_holidays = deque()

        for provided_holiday in map(lambda date: date.strip(), provided_holidays_list):
            date_range_idx = provided_holiday.find("-")

            if date_range_idx == -1:
                all_holidays.append(provided_holiday)
                continue

            min_day, max_day = self.__extract_minmax_days(provided_holidays_list, provided_holiday, date_range_idx)
            all_holidays.extend(self.__extract_holidays_from_range(provided_holiday, min_day, max_day))

        return all_holidays

    def __extract_minmax_days(
            self, provided_holidays_list: Iterable[str], provided_holiday: str, sep_idx: int
    ) -> Optional[map[str, str]]:
        try:
            return map(
                lambda date: match("\d+", date.strip()).group(),  # extract day number from <day> <month>
                (provided_holiday[:sep_idx], provided_holiday[sep_idx + 1:]),  # min_date, max_date
            )
        except AttributeError:
            print(f"Проверьте правильность заполнения дат для skip_holidays_list: {provided_holidays_list}")
            print("Список не сформирован")

    def __extract_holidays_from_range(
            self, provided_holiday: str, min_day: str, max_day: str
    ) -> Optional[Generator[str, str, None]]:
        date_month = provided_holiday[-3:]

        return (f"{day} {date_month}" for day in range(int(min_day), int(max_day) + 1))
    ...
    def _preprocess_data(self, df, time_format, time_errors, dayfirst, yearfirst):
        ...
        if self._id_column is not None:
            ...
            for col in [self._id_column]:
                if col is not None:
                    ...
                    if mask.sum() > 0:
                        warn(
                            f"DataHolder: column {col} has {mask.sum()} None values, "
                            f"the corresponding rows will be removed.",
                            UserWarning,
                        )
        ...
        for col in [self._id_column, self.activity_column, self.user_column, self.text_column]:
            if col is not None and not is_string_dtype(col):
                df[col] = df[col].astype(str)

        # Convert time to pd.date_time
        for time_column in [self.start_timestamp_column, self.end_timestamp_column]:
            if time_column is not None and not is_datetime64_any_dtype(time_column):
                if time_format is None:
                    warn(
                        "DataHolder: 'time_format' is not set, "
                    ...
                    df[time_column] = to_datetime(
                        df[time_column], dayfirst=dayfirst, yearfirst=yearfirst, utc=self.utc
                    )

                elif time_errors == "raise":
                    df[time_column] = to_datetime(df[time_column], format=time_format, errors="raise", utc=self.utc)
                elif time_errors == "coerce":
                    df[time_column] = to_datetime(df[time_column], format=time_format, errors="coerce", utc=self.utc)
                elif time_errors == "auto_convert":
                    dayfirst, yearfirst = DataHolder._check_dayfirst_yearfirst(dayfirst, yearfirst)
                    result = to_datetime(df[time_column], format=time_format, errors="coerce", utc=self.utc)
                    na_mask = result.isna()
                    non_converted_timestamps = df[time_column][na_mask]
                    res = to_datetime(non_converted_timestamps, dayfirst=dayfirst, yearfirst=yearfirst, utc=self.utc)
                    df[time_column] = result
                    df.loc[na_mask, time_column] = res
                else:
                    raise ValueError(
                        "time_errors must be in "
    ...
    def _warn_not_set(argument):
        warn(
            f"DataHolder: timestamp auto conversion will be done. '{argument}' "
            "is not set, in ambiguous cases it will be considered as False.",
            UserWarning,
        )

    def __filter_out_weekends(self, timestamp_column: str) -> None:
        workday_mask = self.data[timestamp_column].dt.weekday < 5  # 0 stands for Monday, as 5 stand for Saturday

        self.data = self.data[workday_mask]  # only keep stages on workdays

    def __filter_out_user_holidays(self, timestamp_column: str) -> None:
        def get_days_as_string(datetime_series: Series):
            return datetime_series.dt.day.astype("int").astype("str")

        def shorten_month_name(full_name: str) -> str:
            return full_name[:3]

        def get_short_months_as_string(datetime_series: Series) -> Series:
            return datetime_series.dt.month_name().map(shorten_month_name)

        def construct_series_of_day_month(datetime_series: Series) -> Series:
            return get_days_as_string(datetime_series).str.cat(
                get_short_months_as_string(datetime_series), sep=" "
            )

        self.data = self.data[
            ~construct_series_of_day_month(self.data[timestamp_column]).isin(self.skip_holidays_list)
        ]

    def __adjust_calendar_days(self) -> None:
        timestamp_columns_filtered = filter(None, (self.start_timestamp_column, self.end_timestamp_column))

        for timestamp_column in timestamp_columns_filtered:
            if not self.keep_weekends:
                self.__filter_out_weekends(timestamp_column)
            if self.skip_holidays_list:
                self.__filter_out_user_holidays(timestamp_column)
    ...
    def get_grouped_data(self, *columns):
        ...
        columns_to_agg = (
            ...
            else [col for col in columns if col not in self.grouped_data.columns]
        )
        if len(columns_to_agg) != 0:
            ...
            if self.n_jobs == 1:
                ...
                result_objects = [
                    pool.apply_async(self._groupby, args=(sub_data, self.id_column, *columns_to_agg))
                    for sub_data in generate_data_partitions(self.data, self.id_column, batch_num=self.n_jobs * 2)
                ]
                grouped_result = concat([r.get() for r in result_objects])
                pool.close()
                pool.join()
            # Save the result
            if self.grouped_data is None:
    ...
    def check_or_calc_duration(self):
        ...
        """
        if self.duration_column is not None:
            return
        ...
        if start_timestamp_column is None and end_timestamp_column is None:
            raise RuntimeError(
                "Cannot calculate time difference, "
        ...
        df[duration_column] = df[duration_column] / Timedelta(seconds=1)
        self.data = df
        self.duration_column = duration_column
    ...
    def is_interval(self) -> bool:
        ...
        is_interval: bool
        """
        return self.start_timestamp_column is not None and self.end_timestamp_column is not None
    ...
    def top_traces_dh(self, n: int) -> "DataHolder":
        ...
        id_series = (
            dfg.groupby(self.activity_column)
